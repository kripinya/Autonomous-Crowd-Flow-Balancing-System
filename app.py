...
def get_phase(): return event_context['phases'][event_context['phase_idx']]
def get_weather(): return event_context['conditions'][event_context['weather_idx']]
...
def classify_risk(gate):
    level = get_density_level(gate['density'])
    if level == 'High' and gate['inflow'] > gate['outflow']: return 'HIGH'
    if level == 'High' or level == 'Medium': return 'MODERATE'
    return 'SAFE'

def is_congestion_risk(gate):
...
def simulate_gate(gid, gate):
    weather = get_weather()
    phase = get_phase()
    
    inflow_mod = 0
    if 'Pre-Match' in phase: inflow_mod += 30
    ...
    if gate['inflow'] > gate['outflow']: gate['streak'] += 1
    else: gate['streak'] = max(0, gate['streak'] - 1)

def evaluate_gate(gid, gate):
    ...
    if risk == 'HIGH':
        ...
        if 'Rain' in weather and name == 'East Gate': ctx = f"{name} surging unexpectedly due to rain."
        if 'Pre-Match' in phase: ctx = f"{name} bottleneck severe prior to kickoff."
